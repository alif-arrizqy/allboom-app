...
const getImageUrl = (
    ...
    switch (type) {
        case "thumbnail":
            return (
                submission.imageThumbnail ||
                submission.imageMedium ||
                submission.imageUrl
            );
        case "medium":
...
const Assignments = () => {
    ...
    const [selectedStudentAssignment, setSelectedStudentAssignment] =
        useState<StudentAssignmentView | null>(null);
    ...
    const [deleteAction, setDeleteAction] = useState<"delete" | "draft">(
        "delete",
    );
    ...
    const [isStudentDetailDialogOpen, setIsStudentDetailDialogOpen] =
        useState(false);

    // Materi states
    const [materiInputType, setMateriInputType] = useState<
        "none" | "file" | "link"
    >("none");
    ...
    const [isEditSubmissionDialogOpen, setIsEditSubmissionDialogOpen] =
        useState(false);
    const [isCancelSubmissionDialogOpen, setIsCancelSubmissionDialogOpen] =
        useState(false);
    ...
    const getAssignmentWithSubmissions = (
        assignment: Assignment,
        ...
        const totalStudents =
            assignment.classes?.reduce((sum, ac) => {
    ...
    const getStudentAssignmentView = (
        assignment: Assignment,
        ...
        return {
            ...assignment,
            status: submission.status.toLowerCase() as
                | "pending"
                | "revision"
                | "graded"
                | "not_submitted",
    ...
    const filteredAssignments = (
        assignments.map((assignment) =>
            isTeacher
                ...
                : getStudentAssignmentView(assignment),
        ...
        if (isTeacher) {
            ...
            return (
                matchesSearch &&
                mappedStatus &&
                assignment.status === mappedStatus
            );
        } else {
        ...
        try {
            ...
            if (materiInputType === "file" && materiFile) {
                ...
                const uploadResponse =
                    await assignmentService.uploadMateriFile(materiFile);
                setMateriUploading(false);
                if (!uploadResponse.success) {
                    toast({
                        title: "Gagal Upload Materi",
                        description: "Gagal mengupload file materi",
                        variant: "destructive",
                    });
                    return;
                }
            ...
            if (response.success) {
                toast({
                    title: "Tugas Dibuat! 📝",
                    description:
                        "Tugas baru berhasil dibuat dan dikirim ke siswa",
                });
    ...
    const handleSubmitAssignment = async (e: React.FormEvent) => {
        ...
        if (
            ...
            !submissionFormData.title ||
            !submissionFormData.image
        ) {
            toast({
                title: "Data Tidak Lengkap",
        ...
        try {
            ...
            if (response.success) {
                toast({
                    ...
                    description:
                        "Karya berhasil dikumpulkan untuk dinilai guru",
                });
    ...
    const handleReturnForRevision = async () => {
        ...
        try {
            setSubmitting(true);
            const response = await submissionService.returnForRevision(
                selectedSubmission.id,
                {
            ...
            if (response.success) {
                toast({
                    ...
                    description:
                        "Siswa akan menerima notifikasi untuk memperbaiki karya",
                });
                setIsReviewDialogOpen(false);
                setSelectedSubmission(null);
                setRevisionNote("");
            ...
            const errorMessage =
                error instanceof Error
                    ? error.message
                    : "Gagal mengembalikan untuk revisi";
            toast({
                title: "Error",
                description: errorMessage,
                variant: "destructive",
    ...
    const handleConfirmDelete = async () => {
        ...
        try {
            ...
            if (deleteAction === "delete") {
                const response = await assignmentService.deleteAssignment(
                    selectedAssignment.id,
                );
                if (response.success) {
                    toast({
    ...
    const handleDeleteMediaType = async (mediaTypeId: string) => {
        ...
        try {
            setSubmitting(true);
            const response =
                await mediaTypeService.deleteMediaType(mediaTypeId);
            if (response.success) {
                toast({
    ...
    const handleSelectAll = () => {
        if (selectedAssignments.length === filteredAssignments.length) {
            ...
            setSelectedAssignments(
                filteredAssignments.map((a: ExtendedAssignment) => a.id),
            );
        }
    };
    ...
    const handleSaveEditSubmission = async (e: React.FormEvent) => {
        e.preventDefault();
        const submission = selectedStudentAssignment
            ? getStudentSubmission(selectedStudentAssignment.id)
            : null;
        if (!submission) return;
        ...
        try {
            ...
            if (response.success) {
                toast({
                    ...
                    description:
                        "Karya berhasil dibatalkan dan dapat dikumpulkan kembali",
                });
    ...
    const getRevisionCount = (assignment: AssignmentWithSubmissions) => {
        return (
            assignment.studentSubmissions?.filter(
                (s: Submission) => s.status === "REVISION",
            ).length || 0
        );
    };

    // Helper: render materi badge for assignment cards
    const renderMateriBadge = (assignment: {
    ...
    const renderMateriField = () => (
        <div className="space-y-2">
            ...
            <p className="text-xs text-muted-foreground">
                Upload file PPT, DOC, PDF atau tambahkan link Google Drive /
                YouTube
            </p>
            {/* Toggle buttons */}
                        ...
                        {materiFile || materiFileName ? (
                            <div className="flex items-center justify-center gap-2">
                                ...
                                <span className="text-sm font-medium text-primary truncate max-w-[200px]">
                                    {materiFile?.name || materiFileName}
                                </span>
                                <button
                                    type="button"
            ...
            {materiInputType === "link" && (
                <div className="space-y-1">
                    ...
                    <p className="text-xs text-muted-foreground">
                        Contoh: Google Drive, YouTube, atau link file lainnya
                    </p>
                </div>
            )}
        </div>
        ...
        <div className="space-y-6">
                                        ...
                                        >
                                            ...
                                            <div className="space-y-2">
                                                ...
                                                <Textarea
                                                    ...
                                                    value={
                                                        editAssignmentData.description
                                                    }
                                                    onChange={(e) =>
                                                        setEditAssignmentData({
                                                            ...editAssignmentData,
                                            ...
                                            <div className="space-y-2">
                                                ...
                                                <div className="flex gap-2">
                                                    ...
                                                    <Popover
                                                        open={isAddingMediaType}
                                                        onOpenChange={
                                                            setIsAddingMediaType
                                                        }
                                                    >
                                            ...
                                            <div className="space-y-2">
                                                <Label>
                                                    Ukuran Karya (opsional)
                                                </Label>
                                                <Input
                                                    placeholder="cth: 30x40cm"
                                                    className="rounded-xl"
                                                    value={
                                                        editAssignmentData.artworkSize
                                                    }
                                                    onChange={(e) =>
                                                        setEditAssignmentData({
                                                            ...editAssignmentData,
                                                                ...
                                                                (cls) => (
                                                                    ...
                                                                    >
                                                                        <input
                                                                            type="checkbox"
                                                                            id={`class-${cls.id}`}
                                                                            checked={editAssignmentData.classIds.includes(
                                                                                cls.id,
                                                                            )}
                                                                            onChange={(
                                                                                e,
                                                                                    ...
                                                                                    setEditAssignmentData(
                                                                                        {
                                                                                            ...editAssignmentData,
                                                                                            classIds:
                                                                                                editAssignmentData.classIds.filter(
                                                                                                    (
                                                                                                        id,
                                                                                                    ) =>
                                                                                                        id !==
                ...
                <div className="space-y-4">
                    {filteredAssignments.map(
                        (assignment: ExtendedAssignment) => (
                            <Card
                                key={assignment.id}
                                className={`card-playful group ${isBulkMode && isTeacher ? "border-2" : ""} ${selectedAssignments.includes(assignment.id) ? "border-primary" : ""}`}
                            >
                                                            ...
                                                            {assignment.classes
                                                                ...
                                                                .map((name) => (
                                                                    <span
                                                                        ...
                                                                        className="text-xs bg-primary/10 text-primary px-2 py-0.5 rounded-lg font-medium"
                                                                    >
                                                                        {name}
                                                                    </span>
                                                                ))}
                                                            ...
                                                            <Button
                                                                ...
                                                                onClick={() => {
                                                                    ...
                                                                    const pendingSubmission =
                                                                        assignment.studentSubmissions?.find(
                                                                            (
                                                                                s: Submission,
                                                                            ) =>
                                                                                s.status ===
                                ...
                                <div className="space-y-5">
                                    ...
                                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                                        ...
                                        <div className="p-3 bg-muted/50 rounded-xl">
                                            ...
                                            <p className="font-semibold text-sm mt-0.5">
                                                {assignmentWithSubmissions.submissions ||
                                                    0}
                                                /
                                                {assignmentWithSubmissions.total ||
                                                    0}
                                            </p>
                                        </div>
                                        <div className="p-3 bg-secondary/10 rounded-xl">
                                            ...
                                            <p className="font-semibold text-sm text-secondary mt-0.5">
                                                {assignmentWithSubmissions.pendingCount ||
                                                    0}
                                            </p>
                                        </div>
                                        <div className="p-3 bg-accent/10 rounded-xl">
                                            ...
                                            <p className="font-semibold text-sm text-accent mt-0.5">
                                                {assignmentWithSubmissions.revisionCount ||
                                                    0}
                                            </p>
                                        </div>
                                    </div>
                                            ...
                                            selectedAssignment.classes.length >
                                                ...
                                                >
                                                    <TabsList className="h-9 rounded-xl bg-muted/50 flex-wrap">
                                                        ...
                                                        {selectedAssignment.classes.map(
                                                            (c) => (
                                                                <TabsTrigger
                                                                    key={
                                                                        c.class
                        ...
                        (() => {
                            ...
                            return (
                                <div className="space-y-4">
                                    ...
                                    {selectedStudentAssignment.status ===
                                        "revision" &&
                                        selectedStudentAssignment.revisionNote && (
                                            <div className="p-3 bg-accent/10 border border-accent/20 rounded-xl">
                                                <p className="text-xs font-semibold text-accent mb-1">
                                                    Catatan Revisi dari Guru:
                                                </p>
                                    ...
                                    <div className="grid grid-cols-2 gap-3">
                                        <div className="p-3 bg-muted/50 rounded-xl">
                                            ...
                                            <p className="font-semibold text-sm mt-0.5">
                                                {new Date(
                                                    selectedStudentAssignment.deadline,
                                                ).toLocaleDateString("id-ID", {
                                                    day: "numeric",
                                                    month: "long",
                                                    year: "numeric",
                                        ...
                                        {selectedStudentAssignment.classes &&
                                            selectedStudentAssignment.classes
                                                .length > 0 && (
                                                <div className="p-3 bg-muted/50 rounded-xl">
                                                    <p className="text-xs text-muted-foreground">
                    ...
                    {selectedSubmission && (
                        <div className="space-y-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs text-muted-foreground">
                                        ...
                                        {selectedSubmission.revisionCount > 0 &&
                                            ` • Revisi ke-${selectedSubmission.revisionCount}`}
                                    </p>
                                </div>
                            </div>
                                        ...
                                        selectedSubmission.imageHistory.length >
                                            ...
                                            <div className="mt-4 p-3 bg-muted/30 rounded-xl">
                                                ...
                                                <div className="flex flex-wrap gap-2">
                                                    {selectedSubmission?.imageHistory?.map(
                                                        (
                                                            version,
                                                            idx: number,
                                                        ) => (
                            ...
                            <Tabs defaultValue="approve" className="w-full">
                                ...
                                >
                                    <form
                                        onSubmit={handleApproveAndGrade}
                                        className="space-y-4"
                                    >
                                        <div className="space-y-2">
                                        ...
                                        <Button
                                            ...
                                            disabled={
                                                !gradeValue ||
                                                parseInt(gradeValue) < 0 ||
                                                parseInt(gradeValue) > 100
                                            }
                                        >
            ...
            >
                <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
                    ...
                    {selectedStudentAssignment?.status === "revision" &&
                        selectedStudentAssignment?.imageHistory &&
                        selectedStudentAssignment.imageHistory.length > 0 && (
                                ...
                                {selectedStudentAssignment?.imageHistory &&
                                    selectedStudentAssignment.imageHistory
                                        ...
                                        <div className="mt-3 pt-3 border-t border-muted">
                                            ...
                                            <div className="flex flex-wrap gap-2">
                                                {selectedStudentAssignment.imageHistory.map(
                                                    (version, idx: number) => (
                                                        <button
                                                            key={idx}
                                                            type="button"
                    ...
                    >
                        ...
                        <div className="space-y-2">
                            <Label htmlFor="submission-image">
                                Upload Karya
                            </Label>
                            <input
                                id="submission-image"
                            ...
                            >
                                {submissionImagePreview ? (
                                    <div className="space-y-2">
                                        <img
                                            src={submissionImagePreview}
                                            alt="Preview"
                                            className="max-h-48 mx-auto rounded-lg"
                                        />
                                        <p className="text-sm text-muted-foreground">
                                            {submissionFormData.image?.name}
                                        </p>
                                        <Button
                                            type="button"
                                            variant="outline"
                        ...
                        <div className="flex gap-3">
                            ...
                            <Button
                                ...
                                className="flex-1"
                                disabled={
                                    submitting || !submissionFormData.image
                                }
                            >
                                {submitting ? (
                                    <>
                        ...
                        selectedStudentAssignment.status === "graded" && (
                            <div className="space-y-4">
                                ...
                                <div className="text-center p-6 bg-gradient-to-br from-green/10 to-primary/10 rounded-2xl">
                                    ...
                                    <p className="text-5xl font-display font-bold text-green">
                                        {selectedStudentAssignment.grade}
                                    </p>
                                </div>
            ...
            >
                <DialogContent className="sm:max-w-md max-h-[90vh] overflow-y-auto">
                    ...
                    <form
                        onSubmit={handleSaveEditAssignment}
                        className="space-y-4"
                    >
                        <div className="space-y-2">
                            <Label>Judul Tugas</Label>
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="flex gap-2">
                                ...
                                <Popover
                                    open={isAddingMediaType}
                                    onOpenChange={setIsAddingMediaType}
                                >
                                    <PopoverTrigger asChild>
                                        <Button
                                            type="button"
                        ...
                        <div className="flex gap-3">
                            <Button
                                ...
                                disabled={submitting || materiUploading}
                            >
                                Batal
                            </Button>
                            <Button
                ...
                <DialogContent className="sm:max-w-md">
                    ...
                    <div className="space-y-4">
                        <div className="space-y-3 p-4 bg-muted/50 rounded-xl">
                            ...
                            <div className="space-y-2">
                                <label
                                    className="flex items-center gap-3 p-3 rounded-lg border-2 cursor-pointer transition-colors hover:bg-background"
                                    style={{
                                        borderColor:
                                            deleteAction === "draft"
                                                ? "hsl(var(--primary))"
                                ...
                                >
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <p className="text-xs text-muted-foreground mt-1">
                                            Tugas akan disembunyikan dari siswa
                                ...
                                >
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <p className="text-xs text-muted-foreground mt-1">
                                            Tugas akan dihapus selamanya dan
            ...
            <Dialog
                open={isEditSubmissionDialogOpen}
                onOpenChange={setIsEditSubmissionDialogOpen}
            >
                <DialogContent className="sm:max-w-md">
                    <DialogHeader>
                        <DialogTitle className="font-display">
                        ...
                        <DialogDescription>
                            Ubah informasi karya untuk tugas "
                            {selectedStudentAssignment?.title}"
                        </DialogDescription>
                    </DialogHeader>
                    <form
                        onSubmit={handleSaveEditSubmission}
                        className="space-y-4"
                    >
                        <div className="space-y-2">
            ...
            <Dialog
                open={isCancelSubmissionDialogOpen}
                onOpenChange={setIsCancelSubmissionDialogOpen}
            >
                <DialogContent className="sm:max-w-md">
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Apakah Anda yakin ingin membatalkan pengumpulan
                            karya untuk tugas "
                            {selectedStudentAssignment?.title}"? Karya akan
                            dibatalkan dan Anda dapat mengumpulkan kembali.
                        </DialogDescription>
                    </DialogHeader>
                    <div className="flex gap-3">
                        <Button
            ...
            {filteredAssignments.length === 0 && (
                <div className="text-center py-16">
                    ...
                    <p className="text-muted-foreground mb-4">
                        {isTeacher
                            ? "Buat tugas pertamamu untuk siswa"

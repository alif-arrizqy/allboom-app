...
apiClient.interceptors.response.use(
  ...
  async (error: AxiosError) => {
    ...
    // Skip token refresh for auth endpoints (login/register failures should show error directly)
    const isAuthRoute = originalRequest.url?.includes('/auth/login') ||
                        originalRequest.url?.includes('/auth/register');

    // If error is 401 and we haven't tried to refresh yet (and not an auth route)
